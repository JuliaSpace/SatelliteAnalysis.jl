...
function sample_propagator()
    ...
    orbit = KeplerianElements(jd, 7130.982e3, 0.001111, deg2rad(98.405),
        ltdn_to_raan(10.5, jd), deg2rad(90), 0)
    Propagators.init(Val(:J2), orbit)
end
...
measure(
    ...
    () -> ground_facility_accesses(orbp, (0, 0, 0); duration = 3600, step = 300, num_chunks = 1)
)
measure("eclipse_time_summary", () -> eclipse_time_summary(orbp; num_days = 5))
measure("sun-synchronous solver", () -> sun_sync_orbit_from_angular_velocity(14 * 2π / 86400))
