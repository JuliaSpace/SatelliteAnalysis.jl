...
"""
    ground_station_accesses(T, orbp, [(WGS84)], Δt, eci, ecef, vargs...; kwargs...)
...
The ground stations are specified using a vector of tuples with three numbers
...
- `T = Tuple`: The function return a vector of tuples. Each element represent an
    access between the satellite and the ground station. The beginning of the
...
function ground_station_accesses(
    ...
    return ground_station_accesses(T, orbp, [gs_wgs84], vargs...; kwargs...)
end
...
function ground_station_accesses(
    ...
    # Convert the ground station positions to an ECEF vector to save
    ...
    function f(t)::Bool
        ...
        @inbounds for i in eachindex(visibility)
            visibility[i] = is_ground_station_visible(r_e, vgs_r_e[i], θ)
        end
...
"""
    ground_station_gaps(T, orbp, args...; t_0::Number = 0, kwargs...)
...
- `T = Tuple`: The function return a vector of tuples. Each element represent an
    access gap between the satellite and the ground station. The beginning of
...
function ground_station_gaps(
    ...
    return ground_station_gaps(T, orbp, [gs_wgs84], vargs...; kwargs...)
end
